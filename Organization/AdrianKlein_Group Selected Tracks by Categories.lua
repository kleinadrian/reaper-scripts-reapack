-- @description Group Selected Tracks by Categories
-- @version 2.0
-- @author Adrian Klein
-- @about
--   Groups selected tracks into category folders when multiple matches are found.
--   Leaves single tracks ungrouped and applies consistent naming.

--[[
 * ReaScript Name: Group Selected Tracks by Categories
 * Author: Adrian Klein
 * Version: 2.0
 * Description:
 *   Sorts selected tracks by category.
 *   Creates parent folders only for categories with 2 or more tracks.
 *   Single tracks remain ungrouped, 2 or more tracks are grouped into folders.
 *   Parent folders use Title Case, child tracks use lowercase.
 *   Safe cleanup with duplicate numbering.
 *   Newly created folders are collapsed.
 *   Final selection includes parent folders and single tracks only.
]]
...
local function msg(text)
  reaper.ShowMessageBox(text, "Group selected tracks by categories", 0)
end
...
local function build_contiguous_groups(sorted_items)
  local groups = {}
  local current = nil

  for _, item in ipairs(sorted_items) do
    if not current or current.category ~= item.category then
...
if sel_count == 0 then
  msg("Select the tracks you want to group first.")
  return
end
...
local groups = build_contiguous_groups(selected)

reaper.Undo_BeginBlock()
reaper.PreventUIRefresh(1)

-- Step 1: reorder selected tracks in place
...
-- Step 2: create folders only for groups with 2+ tracks
local created_parents = {}
local loose_track_guids = {}

for gi = #groups, 1, -1 do
  local group = groups[gi]
  local count = #group.items
  local need_folder = count >= 2

  if need_folder then
    local first_tr = get_track_by_guid(group.items[1].guid)
    local last_tr = get_track_by_guid(group.items[#group.items].guid)

    if first_tr and last_tr then
      local start_idx = track_index0(first_tr)

      reaper.InsertTrackAtIndex(start_idx, true)
      local parent = reaper.GetTrack(0, start_idx)

      set_track_name(parent, folder_case_name(group.category))
      reaper.SetMediaTrackInfo_Value(parent, "I_FOLDERDEPTH", 1)
      reaper.SetMediaTrackInfo_Value(parent, "I_FOLDERCOMPACT", 2)

      local last_fd = reaper.GetMediaTrackInfo_Value(last_tr, "I_FOLDERDEPTH")
      reaper.SetMediaTrackInfo_Value(last_tr, "I_FOLDERDEPTH", last_fd - 1)

      created_parents[#created_parents + 1] = parent
    end
  else
    for _, item in ipairs(group.items) do
      loose_track_guids[#loose_track_guids + 1] = item.guid
    end
  end
end

-- Step 3: rename original selected tracks only
...
for _, parent in ipairs(created_parents) do
  set_track_name(parent, folder_case_name(get_track_name(parent)))
  reaper.SetMediaTrackInfo_Value(parent, "I_FOLDERCOMPACT", 2)
end
...
for _, guid in ipairs(loose_track_guids) do
...
reaper.Undo_EndBlock("Group selected tracks by categories", -1)